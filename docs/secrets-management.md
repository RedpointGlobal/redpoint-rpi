...
> **Internal service passwords** (Redis, RabbitMQ, Rebrandly Redis, Twilio Redis) are auto-generated by the chart into the `rpi-internal-services` K8s Secret regardless of provider. You do NOT need to store these in your vault, CLI, or SecretProviderClass. This includes: `QueueService_RedisCache_Password`, `QueueService_RedisCache_ConnectionString`, `QueueService_RabbitMQ_Password`, `RealtimeAPI_RedisCache_Password`, `RealtimeAPI_RedisCache_ConnectionString`, `RealtimeAPI_RabbitMQ_Password`, `Rebrandly_RedisPassword`, `TwilioMessaging_RedisCache_Password`. The same-named keys belong in your vault ONLY when the service points at an EXTERNAL Redis/RabbitMQ.
