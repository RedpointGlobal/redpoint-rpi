...
**Note:** Internal service passwords (every chart-deployed Redis cache and RabbitMQ broker: Realtime API, Queue Reader distributed queues, Rebrandly, Twilio Messaging) are auto-generated by the chart. Do not include them in this secret.
...
**Execution Service optional integrations:**

| Key | Required when |
|:----|:------------|
| `NewRelic_ApiKey` | `executionservice.logging.newRelic.enabled: true` |
| `Loggly_ApiKey` | `executionservice.logging.loggly.enabled: true` |
| `PubSub_ConnectionString` | `executionservice.pubSub.enabled: true` |
| `Mercury_Password` | `executionservice.mercury.enabled: true` |
| `ExecutionService_AzureBlob_ConnectionString` | `executionservice.internalCache.provider: azureblob` with `azureStorageSettings.useCloudIdentity: false` |

**Diagnostics mode** (if `diagnosticsMode.enabled: true`):

| Key | Required when |
|:----|:------------|
| `CopyToAzureBlobAccessKey` | `diagnosticsMode.copytoAzureBlob.enabled: true` |
| `CopyToSFTPSecureFTPPassword` | `diagnosticsMode.copytoSftp.enabled: true` |

**Smart Activation** (if `smartActivation.enabled: true`):
...
| `CDP_Keycloak_Admin_Password` | Keycloak administrator password |
| `CDP_Keycloak_Client_Secret` | Keycloak client secret for the `rpi-service` client (also used by the Deployment API when `deploymentapi.useExternalUserManagement: true`) |
| `CDP_Mongo_ConnectionString` | Smart Activation MongoDB connection string |
| `CDP_RabbitMQ_Password` | Smart Activation message queue password |
| `CDP_SIGMA_Client_Secret` | Sigma client secret (if `reportingservice.sigma.enabled: true`) |

> **Internal service passwords** (Redis, RabbitMQ, Rebrandly Redis, Twilio Redis) are auto-generated by the chart into the `rpi-internal-services` K8s Secret regardless of provider. You do NOT need to store these in your vault, CLI, or SecretProviderClass. This includes: `QueueService_RedisCache_Password`, `QueueService_RedisCache_ConnectionString`, `QueueService_RabbitMQ_Password`, `RealtimeAPI_RedisCache_Password`, `RealtimeAPI_RedisCache_ConnectionString`, `RealtimeAPI_RabbitMQ_Password`, `Rebrandly_RedisPassword`, `TwilioMessaging_RedisCache_Password`. The Queue Reader's `QueueService_*` keys are always internal (the chart managed Redis and RabbitMQ deployed when `queuereader.realtimeConfiguration.isDistributed: true`), so they are never stored in your vault. The `RealtimeAPI_*` keys belong in your vault only when the Realtime API points at an EXTERNAL Redis/RabbitMQ (`realtimeapi.cacheProvider.redis.type` or `realtimeapi.queueProvider.rabbitmq.type` set to `external`).
...
The chart deploys internal Redis and RabbitMQ StatefulSets for several services: the Realtime API's cache and queue broker (when their `type` is `internal`), the Queue Reader's distributed queues (`queuereader.realtimeConfiguration.isDistributed: true`), Rebrandly, and Twilio Messaging. These are chart managed infrastructure: their credentials come from `rpi-internal-services` in every secrets mode, and the servers always run with authentication regardless of your secrets provider.
...
| `TwilioMessaging_RedisCache_Password` | Twilio Messaging internal Redis password | `twiliomessaging.enabled: true` with internal Redis |
